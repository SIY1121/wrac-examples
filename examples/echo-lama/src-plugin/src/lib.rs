//! Echo Lama plugin — the entry-point crate for this template.
//!
//! A Delay Lama-style monophonic vowel-morphing synth with a built-in delay. `src-plugin`
//! contains only product-specific logic (parameters, state, DSP, GUI); the messy CLAP ABI
//! and FFI invariants are encapsulated in the separate `wrac_clap_adapter` crate.
//!
//! File layout:
