...
// Logical-pixel bounds applied when clamping a resize request. Same as MIN/MAX above so
// the host cannot drag the pane off the bg.png aspect ratio.
const MIN_LOGICAL_GUI_SIZE: LogicalSize<f64> = LogicalSize::new(396.0, 573.0);
const MAX_LOGICAL_GUI_SIZE: LogicalSize<f64> = LogicalSize::new(396.0, 573.0);
