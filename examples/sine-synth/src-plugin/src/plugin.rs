...
use audio_ports::WracSineSynthAudioPorts;
use note_ports::WracSineSynthNotePorts;
use parameters::{WracGainParameters, bypass_parameter_info};
use state_support::WracGainStateSupport;
use wrac_clap_adapter::{
    ActivateContext, Auv2Descriptor, PluginAudioPorts, PluginCore, PluginCoreContext,
    PluginDescriptor, PluginFeature, PluginGui, PluginNotePorts, PluginParameters, PluginResult,
...
pub(crate) const PLUGIN_DESCRIPTOR: PluginDescriptor = PluginDescriptor {
    ...
    description: "Simple MIDI-controlled sine wave synth",
    // Instrument is the important feature here: hosts use it for track/plugin
    // categorisation and for deciding whether note routing should be offered.
...
pub(crate) struct WracGainPlugin {
    ...
    audio_ports: Arc<WracSineSynthAudioPorts>,
    note_ports: Arc<WracSineSynthNotePorts>,
...
impl WracGainPlugin {
    pub(crate) fn new(context: PluginCoreContext) -> Self {
        ...
        let audio_ports = Arc::new(WracSineSynthAudioPorts::new());
        let note_ports = Arc::new(WracSineSynthNotePorts::new());
...
impl PluginCore for WracGainPlugin {
    ...
    fn activate(&mut self, context: ActivateContext) -> PluginResult<Box<dyn Processor>> {
        // Capture sample rate at activation instead of querying host state from
        // process(). The audio thread should only read immutable processor fields and
        // lock-free parameter state.
        log::debug!(
            "activating audio processor: sample_rate={}, min_frames_count={}, max_frames_count={}",
            context.sample_rate,
