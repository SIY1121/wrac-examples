...
use audio_ports::{
    AudioLayoutStore, WracAnalyzer3DAudioPorts, WracAnalyzer3DConfigurableAudioPorts,
};
use parameters::WracAnalyzer3DParameters;
use state_support::WracAnalyzer3DStateSupport;
...
use crate::analyzer::{FftWorker, ImagerWorker};
use crate::audio::WracAnalyzer3DAudioProcessor;
...
pub(crate) const PLUGIN_DESCRIPTOR: PluginDescriptor = PluginDescriptor {
    ...
    description: "Rich 3D analyzer plugin",
    features: &[
...
pub(crate) struct WracAnalyzer3DPlugin {
    ...
    audio_ports: Arc<WracAnalyzer3DAudioPorts>,
    configurable_audio_ports: Arc<WracAnalyzer3DConfigurableAudioPorts>,
    parameters: Arc<WracAnalyzer3DParameters>,
    ...
    // wrapper re-entry.
    state_support: Arc<WracAnalyzer3DStateSupport>,
    gui_notifier: Arc<GuiStateNotifier>,
}

impl WracAnalyzer3DPlugin {
    pub(crate) fn new(context: PluginCoreContext) -> Self {
        let shared = Arc::new(SharedState::new());
        let audio_layout = Arc::new(AudioLayoutStore::new(2));
        let audio_ports = Arc::new(WracAnalyzer3DAudioPorts::new(audio_layout.clone()));
        let configurable_audio_ports = Arc::new(WracAnalyzer3DConfigurableAudioPorts::new(
            audio_layout.clone(),
        ));
        let parameters = Arc::new(WracAnalyzer3DParameters::new(shared.clone()));
        ...
        let state_support = Arc::new(WracAnalyzer3DStateSupport::new(
            project_state,
            shared.clone(),
            gui.notifier.clone(),
        ));
...
pub(crate) fn create_plugin_core(context: PluginCoreContext) -> Box<dyn PluginCore> {
    ...
    Box::new(WracAnalyzer3DPlugin::new(context))
}

// ---------------------------------------------------------------------------
// PluginCore: plugin lifecycle and the extension capabilities offered
// ---------------------------------------------------------------------------
impl PluginCore for WracAnalyzer3DPlugin {
    ...
    fn activate(&mut self, context: ActivateContext) -> PluginResult<Box<dyn Processor>> {
        ...
        Ok(Box::new(WracAnalyzer3DAudioProcessor::new(
            self.shared.clone(),
            audio_channel_count,
            FftWorker::new(
                context.max_frames_count as usize,
                audio_channel_count as usize,
                self.shared.clone(),
