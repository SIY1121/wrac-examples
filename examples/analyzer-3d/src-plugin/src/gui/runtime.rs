...
const MIN_LOGICAL_GUI_SIZE: LogicalSize<f64> = LogicalSize::new(720.0, 720.0);
const MAX_LOGICAL_GUI_SIZE: LogicalSize<f64> = LogicalSize::new(1080.0, 1080.0);

// Embed the frontend zip only for release builds; debug builds use the Vite dev server.
#[cfg(not(debug_assertions))]
const FRONTEND_ZIP: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/wrac_plugin_gui.zip"));
...
pub(crate) struct WracAnalyzer3DGuiRuntime {
...
impl WxpGuiRuntime for WracAnalyzer3DGuiRuntime {
...
impl Drop for WracAnalyzer3DGuiRuntime {
    fn drop(&mut self) {
        ...
        // `command_handler` is left to field drop order.
