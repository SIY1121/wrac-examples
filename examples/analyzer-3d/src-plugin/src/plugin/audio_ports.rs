...
/// (see [`WracAnalyzer3DAudioProcessor`](crate::audio::WracAnalyzer3DAudioProcessor)). The same
...
pub(super) struct WracAnalyzer3DAudioPorts {
    layout: Arc<AudioLayoutStore>,
}

impl WracAnalyzer3DAudioPorts {
    pub(super) fn new(layout: Arc<AudioLayoutStore>) -> Self {
        Self { layout }
    }
}

impl PluginAudioPorts for WracAnalyzer3DAudioPorts {
...
/// `&mut self` lock (see [`WracAnalyzer3DPlugin`](super::WracAnalyzer3DPlugin)). This does not mean
/// changes are allowed while active — the adapter enforces that this is only called when
/// no `Processor` exists (inactive).
pub(super) struct WracAnalyzer3DConfigurableAudioPorts {
    layout: Arc<AudioLayoutStore>,
}

impl WracAnalyzer3DConfigurableAudioPorts {
    pub(super) fn new(layout: Arc<AudioLayoutStore>) -> Self {
        Self { layout }
    }
}
...
impl PluginConfigurableAudioPorts for WracAnalyzer3DConfigurableAudioPorts {
...
/// semantics that cannot be defined in a generic sample.
