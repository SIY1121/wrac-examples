//! WRAC Analyzer3D — the entry-point crate for this template.
//!
//! A minimal analyzer plugin. `src-plugin` contains only product-specific logic
...
//! - `audio.rs`    : DSP running on the audio thread.
//! - `analyzer.rs` : non-realtime analyzer workers running on separate threads..
