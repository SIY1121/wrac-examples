...
pub(crate) use notifier::{GuiStateNotifier, GuiSubscriptionId, editor_page_payload};

use runtime::{
    DEFAULT_GUI_SIZE, GuiRuntimeDependencies, MAX_GUI_SIZE, MIN_GUI_SIZE, WracAnalyzer3DGuiRuntime,
};
...
pub(crate) fn create_gui_integration(
    ...
    let controller = Arc::new(WxpGuiController::new_with_resize_handle(
        move |configuration, initial_size, parent| {
            WracAnalyzer3DGuiRuntime::create(
